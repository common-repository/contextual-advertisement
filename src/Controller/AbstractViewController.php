...
namespace Context360\Controller;
...
abstract class AbstractViewController
{
	 ...
	 *
	 * @param string $pathToScript - path relative to plugin (context360) directory, like: '/assets/js/request.js'.
	 */
	protected function addPluginScript($pathToScript)
	{
		$scriptId = self::$scriptsCounter ++;
		wp_enqueue_script(
			'context360_script_' . $scriptId,
	...
	protected function addPluginAssetScript($url, $addInFooter = false)
	{
		$scriptId = self::$scriptsCounter ++;
		wp_enqueue_script(
			'context360_asset_script_' . $scriptId,
			$url,
			[],
			$this->version,
			$addInFooter
	...
	{
		$scriptId = self::$scriptsCounter ++;
		\wp_add_inline_script('context360_script_' . $scriptId, $scriptContent, 'after');
	}
